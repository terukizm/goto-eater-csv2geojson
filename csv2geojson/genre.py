...
def classify(genre_name: str):
    ...
    # 5: うどん・そば・ラーメン・餃子
    if re.search('ラーメン|らーめん|つけめん|そば|蕎麦|うどん|ちゃんぽん|麺|麵|中華そば', genre_name):
    ...
    # 3: 洋食・フレンチ・イタリアン,
    if re.search('洋食|欧風|オムライス|シチュー|フランス|フレンチ|イタリア|ドイツ|イギリス|スペイン|西洋|ヨーロッパ' +
        '|スパゲティ|ハンバーグ|パスタ|ピザ|ピッツァ|スパゲッティ|ビストロ|アメリカ|ロシア|地中海|ポルトガル|ハワイアン', genre_name):
    ...
    if re.search('アジア|エスニック|韓国|朝鮮|無国籍|多国籍|南米|各国|インド|カレー|メキシコ|ブラジル|アフリカ' +
        '|ベトナム|トルコ|タイ料理|フォー|ネパール|創作', genre_name):
        # MEMO: 山形県は「居酒屋・創作料理」というジャンルがあるが、居酒屋に寄せた
        return GENRE_各国料理

    # 9: カフェ・スイーツ
    if re.search('カフェ|Cafe|パーラー|スイーツ|コーヒー|クレープ|パンケーキ|喫茶|甘味|珈琲|紅茶|茶房' +
        '|パフェ|チョコレート|アイスクリーム|菓子|デザート|ケーキ|ドーナツ|ジェラート', genre_name):
        return GENRE_カフェ
