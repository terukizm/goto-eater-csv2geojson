...
def classify(genre_name: str):
    ...
    # 5: うどん・そば・ラーメン・餃子
    if re.search('ラーメン|らーめん|つけめん|そば|蕎麦|うどん|ちゃんぽん|きしめん|麺|麵|中華そば', genre_name):
        # MEMO: "中華そば"という文字列を"中華"より先にmatchさせる必要があり、さらに"焼きそば"というは
        # ここではmatchさせない(その他を優先)するようにしないといけない
        # MEMO: 栃木県など「うどん・そば・丼」というジャンルがあり、うどん/そば = 麺類　と 丼 = ファーストフード が混ざってるので
        # 悩んだ結果麺類を優先させた
        return GENRE_麺類

    # 8: ファーストフード・ファミレス・食堂
    if re.search('ハンバーガー|ファーストフード|ファストフード|ファミレス|レストラン|バイキング|ドライブイン|フードコート|ブッフェ' +
        '|定食|食事処|食堂|フライドチキン|から揚げ|からあげ|ザンギ|サンドイッチ|サンドウィッチ|丼|どんぶり|軽食|弁当', genre_name):
        # MEMO: "ハンバーガー"が"バー"と誤Hitするので判定順を前に
        # MEMO: 「丼物」を麺類系からファストフード・食堂系にジャンル変更
        return GENRE_ファミレス

    # 3: 洋食・フレンチ・イタリアン,
    if re.search('洋食|欧風|欧州|西洋|オムライス|シチュー|フランス|フレンチ|イタリア|ドイツ|イギリス|スペイン|ギリシャ|ヨーロッパ' +
        '|スパゲティ|ハンバーグ|パスタ|ピザ|ピッツァ|ピッツェリア|スパゲッティ|ビストロ|アメリカ|ロシア|地中海|ポルトガル|ハワイアン', genre_name):
        # MEMO: "ハンバーグ"が"バー"と誤Hitするので判定順を前に (飲み屋系より洋食系が先にHitしてしまうが、副作用ないか…？)
        # スパゲッティは麺類なのか、何もわからない
        return GENRE_洋食

    # 7: ステーキ・鉄板焼・焼肉・ホルモン,
    if re.search('焼肉|焼き肉|ステーキ|鉄板|ホルモン|もつ焼|もつやき|ジンギスカン|牛たん|牛タン|牛肉', genre_name):
        # 富山県の「焼き鳥・焼肉」というジャンルを、焼肉側に倒すためにこの判定順
        return GENRE_焼肉

    # 1: 居酒屋・バー・ダイニングバー・バル
    if re.search('居酒屋|バル|バー|BAR|Bar|酒場|ビヤホール|ビアホール|ビアガーデン|ダイニング' +
        '|カクテル|ビール|ワイン|日本酒|酒類|ハイボール|呑み|宴会|屋形船|パブ|スナック|カラオケ|クラブ|ラウンジ' +
        '|やきとん|やきとり|ヤキトリ|焼鳥|焼き鳥|焼きとり|鳥料理|串揚|串カツ|串かつ|串焼|炉端焼|炭火焼|牡蠣|BBQ', genre_name):
        # MEMO: ラウンジっていう風営法？的なジャンルがあるらしいが、空港とかにある待合室的なラウンジも含まれてしまう気もする
        return GENRE_居酒屋

    # 2: 和食
    if re.search('和食|和風|日本料理|郷土料理|沖縄|九州|京料理|懐石|会席|割烹|料亭|小料理|天ぷら|天麩羅|刺身|精進' +
        '|うなぎ|鰻|ふぐ|はも|うに|すっぽん|あなご|あんこう|すき焼き|しゃぶしゃぶ|川魚|魚料理|鶏料理|ひっつみ' +
        '|とんかつ|かに料理|海鮮|おにぎり|おむすび|お茶漬け|釜飯|おでん|鍋|ちゃんこ|水炊き|すし|寿司|鮨|ひつまぶし', genre_name):
        # もうわかんなくなってきた...
    ...
    # 4: 中華
    if re.search('中華|中国|台湾|四川|広東|上海|点心|飲茶|餃子|薬膳', genre_name):
        # MEMO: 餃子は単体では中華になるように、たいていは「ラーメン・餃子」なのでラーメンが麺類で先にHitするはず
        # 日高屋は何屋なんだという問題には目をつぶっていく
        return GENRE_中華

    # 6: カレー・アジア・エスニック・各国料理
    if re.search('アジア|エスニック|韓国|朝鮮|無国籍|多国籍|南米|中東|各国|インド|カレー|カリー|メキシコ|メキシカン|ブラジル|アフリカ' +
        '|ベトナム|トルコ|タイ料理|フォー|シンガポール|ネパール|創作', genre_name):
        # MEMO: 山形県は「居酒屋・創作料理」というジャンルがあるが、居酒屋に寄せた
        return GENRE_各国料理

    # 9: カフェ・スイーツ
    if re.search('カフェ|Cafe|CAFE|パーラー|スイーツ|コーヒー|クレープ|パンケーキ|喫茶|甘味|珈琲|紅茶|茶房' +
        '|パフェ|チョコレート|アイスクリーム|菓子|デザート|ケーキ|ドーナツ|ジェラート|ジュース|ドリンク', genre_name):
        return GENRE_カフェ
