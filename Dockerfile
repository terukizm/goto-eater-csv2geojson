...
RUN cd /opt \
  ...
  && ldconfig && ldconfig -v | grep dams \
  && make dic && make install-dic \
  # MEMO: pydamsを使う上では、インストールされた/usr/local/lib/dams/* 以外のファイルは必須ではないので削除
...
# install pydams
RUN git clone https://github.com/hottolink/pydams.git /opt/pydams \
  && cd /opt/pydams \
  && make all \
  && rm -rf /opt/pydams
